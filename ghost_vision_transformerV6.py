...
class Ghost_Mlp(nn.Module):
    """ MLP as used in Vision Transformer, MLP-Mixer and related networks
    """
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., ghost_mlp=1):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        drop_probs = to_2tuple(drop)

        hidden_features_ori = hidden_features
        self.ghost_mlp = ghost_mlp

        ratio = hidden_features_ori // in_features
        left_ratio = ratio - self.ghost_mlp
        self.left_ratio = left_ratio
        assert 0 < left_ratio <= ratio, "keep_rate must > 0 and <= 1, got {0}".format(left_ratio)

        self.hidden_features = left_ratio * in_features
        cheap_features = self.ghost_mlp * in_features
        self.cheap_operation = nn.Sequential(
            nn.Conv2d(self.hidden_features, cheap_features, 3, 1, 1, groups=in_features, bias=False),
            nn.BatchNorm2d(cheap_features),
            nn.ReLU(inplace=True),)

        self.cls_token_proj = nn.Conv1d(self.hidden_features, cheap_features, 1, 1, 0, groups=in_features, bias=False)

        self.fc1 = nn.Linear(in_features, self.hidden_features)
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])
        self.fc2 = nn.Linear(hidden_features_ori, out_features)
        self.drop2 = nn.Dropout(drop_probs[1])

    def forward(self, x):
        B, N, C = x.shape
        H = W = int(np.sqrt(N-1)) #remove cls_token
        x = self.fc1(x)
        x = self.act(x)
        ...
        x_ = x[:, 1:]
        x_class = x[:, 0:1]
        x_ = x_.transpose(1, 2).view(B, self.hidden_features, H, W)
        x_ghost = self.cheap_operation(x_)
        x = torch.cat([x_,x_ghost], dim=1) #add ghost  B C H W
        x = x.flatten(2).transpose(1, 2) #B N-1 C
        x_cls = self.cls_token_proj(x_class.transpose(1,2)) #B 1 ghostdim
        x_cls = x_cls.transpose(1,2)
        x_cls = torch.cat([x_cls,x_class], dim=-1) #add ghostClsToken B 1 C
        x = torch.cat([x_cls,x], dim=1) #add cls_token B N C
...
class GhostHead_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., ghost_head=1):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
        self.num_heads = num_heads

        self.ghost_head = ghost_head
        left_heads = num_heads - self.ghost_head
        self.left_heads = left_heads
        assert 0 < left_heads <= num_heads, "keep_rate must > 0 and <= 1, got {0}".format(left_heads)

        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5

        dim_qkv = self.left_heads * head_dim
        self.dim_qkv = dim_qkv
        self.dim = dim_qkv
        self.dim_ghost = self.ghost_head * head_dim
        self.cheap_operation = nn.Sequential(
            nn.Conv2d(dim_qkv, self.dim_ghost, 3, 1, 1, groups=head_dim, bias=False),
            nn.BatchNorm2d(self.dim_ghost),
            nn.ReLU(inplace=True),)
        self.qkv = nn.Linear(dim, dim_qkv * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        # self.cls_token_proj = nn.Linear(dim_qkv, self.dim_ghost)
        self.cls_token_proj = nn.Conv1d(dim_qkv, self.dim_ghost, 1, 1, 0, groups=head_dim, bias=False)
    ...
    def forward(self, x):
        ...
        if self.left_heads <= self.num_heads:  # double check the keep rate
            left_heads = self.left_heads

            qkv = self.qkv(x).reshape(B, N, 3, left_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

            attn = (q @ k.transpose(-2, -1)) * self.scale
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
            x_attn = (attn @ v).transpose(1, 2).reshape(B, N, self.dim_qkv)
            H = W = int(np.sqrt(N-1)) #remove cls_token
            x_attn_ = x_attn[:, 1:]
            x_attn_class = x_attn[:, 0:1]
            x_attn_ = x_attn_.transpose(1, 2).view(B, self.dim_qkv, H, W)
            x_ghost = self.cheap_operation(x_attn_)
            x = torch.cat([x_attn_,x_ghost], dim=1) #add ghost  B C H W
            x = x.flatten(2).transpose(1, 2) #B N-1 C
            x_cls = self.cls_token_proj(x_attn_class.transpose(1,2)) #B ghostdim 1 
            x_cls = x_cls.transpose(1,2) #B 1 ghostdim 
            x_cls = torch.cat([x_cls,x_attn_class], dim=-1) #add ghostClsToken B 1 C
            x = torch.cat([x_cls,x], dim=1) #add cls_token B N C
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, ghost_head=1, ghost_mlp=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = GhostHead_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, ghost_head=ghost_head)
        ...
        self.mlp = Ghost_Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, ghost_mlp=ghost_mlp)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, ghost_head=1, ghost_mlp=1, block_fn=Block):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, ghost_head=self.ghost_head, ghost_mlp=self.ghost_mlp)
...
def _create_deit(variant, pretrained=False, distilled=False, default_cfg=None, **kwargs):
...
@register_model
def ghostFinalBestHeadMlp_deit_tiny_patch16_224(pretrained=False, **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, ghost_head=1, ghost_mlp=1, **kwargs)
    model = _create_deit('deit_tiny_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def ghostFinalBestHeadMlp_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, ghost_head=1, ghost_mlp=1, **kwargs)
